...
def part_3(C):
	...
	print('When C is {}, accuracy rate for training set: {}'.format(C, (predictions == y).mean() * 100.0))
	# 加载测试集
	...
	predictions = predictions.reshape(np.shape(predictions)[0], 1)
	print('When C is {}, accuracy rate for testing set: {}'.format(C, (predictions == y_test).mean() * 100.0))
...
def part_4(C):
	...
	word_indices1, _ = email_preprocess('spamSample1.txt')
	features1 = feature_extraction(word_indices1).transpose()
	print('The soft of spamSample1: {}'.format(linear_svm.predict(features1)))

	word_indices2, _ = email_preprocess('spamSample2.txt')
	features2 = feature_extraction(word_indices2).transpose()
	print('The soft of spamSample2: {}'.format(linear_svm.predict(features2)))

	word_indices1, _ = email_preprocess('emailSample1.txt')
	features1 = feature_extraction(word_indices1).transpose()
	print('The soft of emailSample1: {}'.format(linear_svm.predict(features1)))

	word_indices2, _ = email_preprocess('emailSample2.txt')
	features2 = feature_extraction(word_indices2).transpose()
	print('The soft of emailSample2: {}\n\n'.format(linear_svm.predict(features2)))
...
part_1()
part_2()
C_values = [0.01, 0.03, 0.05, 0.1, 0.3, 1]
for c in C_values:
	part_3(c)
	part_4(c)
