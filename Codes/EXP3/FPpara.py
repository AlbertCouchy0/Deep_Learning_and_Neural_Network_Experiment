...
################# 二元版 #################
def poly_params_search_dual(X, y, X_val, y_val, degree):
    # np.c_values = [0.01,  0.1,  1, 3, 10, 30, 100]
    # sigma_values = [0.01, 0.1,  1, 3, 10, 30, 100]
    np.c_values = [1, 3, 10, 30, 100, 300]
    sigma_values = [1, 0.3, 1, 3, 10]
    best1 = {'C': 0.0001, 'sigma': 0.0001, 'val_error': 999, 'train_error': 999}
    best2 = {'C': 0.0001, 'sigma': 0.0001, 'val_error': 999, 'train_error': 999}
...
#############
def poly_train(X, y, X_val, y_val, C, sigma, degree):
    raveled_y = y.ravel()
    m_val = np.shape(X_val)[0]
    m = np.shape(X)[0]
...
def poly_params_search_C(X, y, X_val, y_val, last_best1, last_C_sec, degree):
    c1 = last_best1['C']
    c2 = last_C_sec
    if c1 > c2:
        temp = c1
    ...
    for i in range(3):
        val_error[i], train_error[i] = poly_train(X, y, X_val, y_val, np.c_values[i], sigma, degree)
...
def poly_params_search_sigma(X, y, X_val, y_val, last_best1, last_sigma_sec, degree):
    s1 = last_best1['sigma']
    s2 = last_sigma_sec
    if s1 > s2:
        temp = s1
    ...
    for i in range(3):
        val_error[i], train_error[i] = poly_train(X, y, X_val, y_val, C, sigma_values[i], degree)
