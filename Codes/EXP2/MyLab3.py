...
from keras.layers import Convolution2D, MaxPooling2D, BatchNormalization, Dense, Flatten, Dropout
cnn = Sequential()
#unit1
# 二维卷积层 和 二维最大池化层 交替堆叠
cnn.add(Convolution2D(filters=64, kernel_size=(3, 3), input_shape=(32, 32, 3), activation='relu',padding='same'))
cnn.add(BatchNormalization())
cnn.add(Convolution2D(filters=64, kernel_size=(3, 3), activation='relu',padding='same'))
cnn.add(BatchNormalization())
cnn.add(MaxPooling2D(pool_size=(2, 2), strides=2, padding='same'))
cnn.add(Dropout(0.2))

cnn.add(Convolution2D(filters=128, kernel_size=(3, 3), activation='relu', padding='same'))
cnn.add(BatchNormalization())
cnn.add(Convolution2D(filters=128, kernel_size=(3, 3), activation='relu', padding='same'))
cnn.add(BatchNormalization())
cnn.add(MaxPooling2D(pool_size=(2, 2), strides=2, padding='same'))
...
cnn.add(Convolution2D(filters=256, kernel_size=(3, 3), activation='relu', padding='same'))
cnn.add(BatchNormalization())
cnn.add(Convolution2D(filters=256, kernel_size=(3, 3), activation='relu', padding='same'))
cnn.add(BatchNormalization())
cnn.add(Convolution2D(filters=256, kernel_size=(3, 3), activation='relu', padding='same'))
...
cnn.add(Convolution2D(filters=512, kernel_size=(3, 3), activation='relu', padding='same'))
cnn.add(BatchNormalization())
cnn.add(Convolution2D(filters=512, kernel_size=(3, 3), activation='relu', padding='same'))
cnn.add(BatchNormalization())
cnn.add(Convolution2D(filters=512, kernel_size=(3, 3), activation='relu', padding='same'))
...
########### 编译模型 ###########
cnn.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-3),loss='categorical_crossentropy', metrics=['acc'])
